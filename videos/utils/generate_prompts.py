...
class PromptGenerator:
    ...
    async def generate_video_script(self, signal: Dict) -> Optional[str]:
        ...
        try:
            ...
            prompt = f"""
            Create a short, engaging TikTok video script about a cryptocurrency trading signal.
            
            Signal Details:
            - Token: {signal.get('token')}
            - Signal Type: {signal.get('signal')}
            - Timeframe: {signal.get('timeframe')}
            - Confidence: {signal.get('confidence')}%
            - Key Levels: {signal.get('keyLevels', 'N/A')}
            - Analysis: {signal.get('reasoning', 'N/A')}
            
            Requirements:
            1. Script should be split into 2 parts with a line break between them
            2. First part should be a catchy intro (2-3 lines)
            3. Second part should highlight the key analysis points (2-3 lines)
            4. Keep it simple and direct
            5. Use emojis sparingly but effectively
            6. Total length should be 4-6 lines
            
            Format the response as plain text with a single \n\n between the two parts.
            """

            # Use the same Claude analysis function from signals.py
            response = analyze_charts_with_claude([], token_info={
                'token': signal.get('token'),
                'analysis': signal.get('reasoning')
            }, custom_prompt=prompt)
            
            if not response or 'script' not in response:
