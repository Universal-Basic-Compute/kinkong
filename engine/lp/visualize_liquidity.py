...
def visualize_liquidity_distribution(data_file, output_file=None):
    """
    Visualize liquidity distribution from JSON data file
    ...
    try:
        ...
        if not distribution:
            logger.error(f"No {x_label.lower()} distribution data found")
            return False
        ...
        ax1.bar(ids, liquidity, alpha=0.7)
        ax1.axvline(x=current_point, color='r', linestyle='--', label=f'Current {x_label}')
        ...
        # Plot price vs bin/tick
        ax2.plot(ids, prices, marker='o', linestyle='-', alpha=0.7)
        ax2.axvline(x=current_point, color='r', linestyle='--', label=f'Current {x_label}')
